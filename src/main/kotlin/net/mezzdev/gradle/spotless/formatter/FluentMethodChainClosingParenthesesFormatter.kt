...
object FluentMethodChainClosingParenthesesFormatter : FormatterFunc, Serializable {
	...
	override fun apply(source: String): String {
		...
		val expandedSource = WrappedChainedCallArgumentFormatter.format(source, originalCompilationUnit)
		...
		return formatClosingParentheses(expandedSource, tokenRange)
	}

	private fun formatClosingParentheses(source: String, tokenRange: TokenRange): String {
		val hasTrailingNewline = source.endsWith('\n')
		val lines = source.removeSuffix("\n").split('\n').toMutableList()
		findCandidates(tokenRange).asReversed().forEach { candidate ->
			formatCandidate(lines, candidate)
		}
		return lines.joinToString("\n") + if (hasTrailingNewline) "\n" else ""
	}

	private fun findCandidates(tokenRange: TokenRange): List<Candidate> {
		return buildList {
